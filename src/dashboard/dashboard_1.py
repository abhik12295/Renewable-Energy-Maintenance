...
# # MEGA URLs for yearly files and models (replace with your MEGA links)
# mega_yearly_urls = {
#     2025: "https://mega.nz/your_link_for_cleaned_data_2025.csv",
#     2024: "https://mega.nz/your_link_for_cleaned_data_2024.csv",
#     # Add URLs for other years if needed
# }
# mega_models = {
#     'prophet_wind': "https://mega.nz/your_link_for_prophet_wind.pkl",
#     'scaler': "https://mega.nz/your_link_for_scaler.pkl",
#     'prophet_solar': "https://mega.nz/your_link_for_prophet_solar.pkl"
# }
...
# # Filters
# energy_source = st.selectbox("Select Energy Source", ['wind', 'solar'])
# view_option = st.radio("Select View Option", ['Historical Data', 'Predicted Data'])
# horizon = st.selectbox("Select Prediction Horizon (for Predicted Data)", ["30 days", "60 days", "90 days", "6 months"])
# horizon_days = {"30 days": 30, "60 days": 60, "90 days": 90, "6 months": 180}
# days = horizon_days[horizon]

# # Data end date (June 30, 2025)
# data_end_date = pd.to_datetime('2025-06-30')

# # Historical data from past 2 months
# past_start = data_end_date - timedelta(days=60)
# past_end = data_end_date
...
#     st.subheader("Historical Power Data (Past 2 Months)")
#     historical = df[['date', 'power_MW']].rename(columns={'power_MW': 'Power (MW)'})
#     st.dataframe(historical)
    
#     # Plot historical
#     st.line_chart(historical.set_index('date')['Power (MW)'])

# else:
#     # Generate future predictions from June 30, 2025
#     st.subheader(f"Predicted Power Data ({horizon})")
...
#                 future[col] = 0
#         forecast = prophet_model.predict(future)
#         future_start = data_end_date + timedelta(days=1)
#         forecast = forecast[forecast['ds'] >= future_start][['ds', 'yhat']].rename(columns={'ds': 'date', 'yhat': 'power_MW'})
#         # Inverse-transform power_MW
#         forecast_scaled = pd.DataFrame({
#             'wind_speed': 0, 'precipitation': 0, 'temperature_avg': 0, 'wind_volatility': 0,
...
# df.to_csv(f"{main_dir}\\data\\processed\\filtered_dashboard_data.csv", index=False)